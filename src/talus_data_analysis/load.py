...
def read_df_from_s3(
    bucket: str, key: str, inputformat: str = None, **df_kwargs
) -> pd.DataFrame:
...
def get_file_keys_in_bucket(bucket: str, key: str, file_type: str = ""):
    ...
    return [
        obj.get("Key")
        for obj in contents
        if _is_file(obj["Key"]) and obj["Key"].endswith(file_type)
    ]
...
def read_excel_from_gdrive(
    gauth, key: str, sheet: str, remove_unnamed: bool = False, **kwargs
) -> pd.DataFrame:
...
def read_csv_from_gdrive(gauth, key: str, **kwargs) -> pd.DataFrame:
